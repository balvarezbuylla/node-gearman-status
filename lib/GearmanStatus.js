...
GearmanFunction = function (name, bufferSize) {       //each function
  ...
  this.rrd                      = new RRD(name+'.rrd');
  this.initialDate              = 0; //seconds from 1970 (UTC)
    ...
    if (data_received.indexOf(".\n")>=0) {   
      ...
      for (functionName in functions) {                         //para cada función
        if (!self.gearmanFunctions[functionName]) {
          ...
          //campos:GAUGE:segundos entre valor y valor y que si no llega lo pone a indefinido:min:max   
         self.gearmanFunctions[functionName].rrd.create (["--no-overwrite","DS:capable_workers:GAUGE:80:0:U", "DS:waiting_jobs:GAUGE:80:0:U", "DS:running_jobs:GAUGE:80:0:U", "RRA:AVERAGE:0.5:1:1000"], {}, function(err) {
            if (err)
               ...
               var date= new Date;
               var origin = Math.round(date.valueOf() / 1000);
               self.gearmanFunctions[functionName].initialDate=origin;
            }
         });

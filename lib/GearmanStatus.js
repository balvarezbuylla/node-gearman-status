  ...
  this.socket.on ('data', function (chunk) {
    ...
    if (data_received.indexOf("\n.\n")>0) {   
      ...
      if (self.first){ 
         ...
         if (self.count%60==2){ //polling each second but save the data each minute. Save higher values of each minute
  ...
  if (!this.full_buffer) {      //buffer is not full at least one time
    for (i=0; i < this.history_workers.write_point; i++){   
      ...
      for (j=0; j < this.number_workers; j++) { 
          if (this.history_workers.circularBuffer[i].data[j].name=='.') {
            return self.error="There isn't worker initiated. Stop the monitoring, init the workers and start the monitoring again.";
          }
          else{

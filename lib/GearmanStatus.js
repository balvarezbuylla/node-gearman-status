...
CircularBuffer.prototype.get = function () {
  ...
  if (this.full) {
    ...
    result = result.concat (0, this.pointer-1);
  }
  else {
...
GearmanFunction.prototype.initMaxValues= function () {
  this.maxCapablesWorkers  = 0;
  this.maxRunningJobs      = 0;
  this.maxWaitingJobs      = 0;
...
GearmanFunction.prototype.addPolling = function (status_function, pollingCount) {
   ...
   if (self.maxCapablesWorkers<status_function.capableWorkers) self.maxCapablesWorkers=status_function.capableWorkers;
   if (self.maxRunningJobs<status_function.runningJobs) self.maxRunningJobs=status_function.runningJobs;
   if (self.maxWaitingJobs<status_function.waitingJobs) self.maxWaitingJobs=status_function.waitingJobs;
...
GearmanFunction.prototype.getHistory =  function () {
   var self=this;
   return {name:self.name, data:self.pollingData.get()};;
